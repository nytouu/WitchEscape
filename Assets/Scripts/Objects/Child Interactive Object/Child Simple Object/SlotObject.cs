...
{
	[SerializeField]
	private bool slotEmpty; // automatiser le true false au start ?
	private GameObject objectOnSlot;

	[SerializeField]
	private GameObject targetObject; //Objet associ� au slot object pour une �nigme
	...
	public override void Interact()
	{
		...
		if (slotEmpty && !interactionScript.GetHandFree()) // si slot vide et objet en main
		{
			...
			//tp l'objet tenu sur le slot, � peaufiner selon l'�paisseur et l'orientation du slot
			objectOnSlot.transform.position = transform.position + Vector3.up * 0.2f;

			CheckGoodObject();
		}
		else if (!slotEmpty && interactionScript.GetHandFree()) // si slot occup� et main vide
		{
